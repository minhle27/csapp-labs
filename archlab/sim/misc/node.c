...
static node_ptr new_node(node_type_t t, int isbool, char* s, node_ptr a1, node_ptr a2)
{
...
static int check_arg(node_ptr arg, int wantbool)
{
    ...
    if (arg->type == N_NUM)
    {
        if (wantbool && strcmp(arg->sval, "0") != 0 && strcmp(arg->sval, "1") != 0)
        {
